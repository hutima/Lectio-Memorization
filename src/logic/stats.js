  ...
  renderBar = (data, a, b, key) => {
    ...
    return h('div', { key, style: { display: 'flex', height: '26px', borderRadius: '7px', overflow: 'hidden', border: '1px solid var(--line)' } }, cells);
  };
  ...
  renderStats = () => {
    ...
    const canon = this.card([
      ...
      this.renderBar(data, 0, data.total, 'all'),
      h('div', { key: 'c', style: { fontSize: '11px', color: 'var(--muted)' } }, 'Genesis → Revelation — every verse in order, equal width.'),
      h('div', { key: 'ot', style: { display: 'flex', flexDirection: 'column', gap: '5px', marginTop: '4px' } }, h('div', { style: { fontSize: '12px', color: 'var(--muted)' } }, 'Old Testament'), this.renderBar(data, 0, data.otEnd, 'otb')),
      h('div', { key: 'nt', style: { display: 'flex', flexDirection: 'column', gap: '5px' } }, h('div', { style: { fontSize: '12px', color: 'var(--muted)' } }, 'New Testament'), this.renderBar(data, data.otEnd, data.total, 'ntb')),
