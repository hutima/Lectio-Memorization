  ...
  // A trailing space or an exact match auto-advances to the next word; Backspace
  // on an empty field steps back to the previous word so a slip can be corrected;
  // tapping any word jumps the cursor there. Each word's value lives in typeVals.
  // ============================================================================
  ...
  onTypeChange = (vi, val) => {
    ...
    this.setState({ typeVals: { ...this.state.typeVals, [vi]: stored } }, () => { if (trailingSpace || exact) this.focusNextType(vi); this.checkTypeDone(); });
  };
  checkTypeDone = () => {
    const p = this.state.passage; if (!p) return; const ws = this.typeWords();
    if (!ws.every((vi) => (this.state.typeVals[vi] || '').trim().length > 0)) return;
    const correct = ws.filter((vi) => this.norm(this.state.typeVals[vi]) === this.norm(p.words[vi].text)).length;
    this.recordResult(correct / ws.length);
  };

  renderWord_type = (s, key) => {
    const h = React.createElement; const vi = s.vi, text = s.text; const st = this.state;
    const val = st.typeVals[vi] || ''; const ok = this.norm(val) === this.norm(text); const filled = val.length > 0;
    ...
    return h('input', {
      ...
      // Tapping a word focuses it directly; keep it visible above the keyboard.
  ...
  vals_type = () => {
    ...
    if (p && ws.length) {
      ...
      if (filled < total) typeStatus = filled + ' / ' + total + ' words';
        ...
        const correct = ws.filter((vi) => this.norm(st.typeVals[vi]) === this.norm(p.words[vi].text)).length;
        typeStatus = correct + ' / ' + total + ' correct';
      }
    }
    return { isType: st.mode === 'type', typeStatus };
  };
